...
-- Return the intersection of s1 and s2
-- Return all the things that are the same in both lists
...
list_difference (h1::t1) l2 = ite (member h1 l2)
                              (list_difference t1 l2)
                              (h1::(list_difference t1 l2))
...
-- Return true if
 --p is true for some v in s, otherwise else false
-- set_exists: (p: a -> bool) -> (s: set a) -> bool
...
toStringset nil = ""
toStringset (h::nil) = (toString h)
toStringset (h::t) = (toString h) ++ "," ++ (toStringset t)

instance (Serialize a) => Serialize (set a) where 
  toString l = "{" ++ (toStringset l) ++ "}"
